export function Header(props) {
    return <header className="flex fixed border-b border-gray-100 bg-white">
        ...
        <div className="flex pl-[820px]">
          ...
          <div className="pl-4 pt-4">
            <button onClick={()=>props.onChange('plus')} className={`border bg-sky-100 border-blue-200 w-10 text-sky-300 cursor-pointer ${props.page === 'plus' ? 'text-blue-600' : 'text-base'}`}>Plus</button>
          </div>
          <div className="pl-4 pt-3">
            <button className="ri-github-line text-2xl cursor-pointer"></button>
          </div>
        </div>
        {props.Header}
    </header>
